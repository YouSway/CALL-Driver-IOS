...
-(void)setFrameWith:(BOOL)isHaveNavControler{
    ...
    self.frame = CGRectMake(toolViewX, toolViewY-64, toolViewW, toolViewH);
}
-(void)setUpPickView{
      CGRect aScreenRect = [[UIScreen mainScreen] bounds];
    UIPickerView *pickView=[[UIPickerView alloc] init];
    //pickView.backgroundColor=[UIColor lightGrayColor];
    pickView.backgroundColor=RGBACOLOR(255, 255, 255, 1);
    _pickerView=pickView;
    pickView.delegate=self;
    pickView.dataSource=self;
    pickView.frame=CGRectMake(0, ZHToobarHeight, aScreenRect.size.width, pickView.frame.size.height);
...
-(UIView*)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    if (!tView){
        ...
        //positionValue.textAlignment = NSTextAlignmentLeft;
        
        if (_isLevelArray) {
            tView.text =_plistArray[component][row];
        }else if (_isLevelString){
            tView.text =_plistArray[row];
        }else if (_isLevelDic){
            NSInteger pIndex = [pickerView selectedRowInComponent:0];
            NSDictionary *dic=_plistArray[pIndex];
            if(component%2==0)
            {
                tView.text =_dicKeyArray[row][component];
            }
            for (id aa in [dic allValues]) {
                if ([aa isKindOfClass:[NSArray class]]&&component%2==1){
                    NSArray *bb=aa;
...
-(void)remove{
    //[self.delegate respondsToSelector:@selector(toobarDonBtnHaveClick:resultString:)];
    //[self.delegate respondsToSelector:@selector(toobarCancelBtnHaveClick:)];
